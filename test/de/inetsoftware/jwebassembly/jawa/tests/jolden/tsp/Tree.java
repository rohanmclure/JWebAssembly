...
import static de.inetsoftware.jwebassembly.jawa.util.Math.*;
...
final class Tree
{
  ...
  {
    puts("x = "); putd(x); puts(" y = "); putd(y); putln();
    for (Tree tmp = next; tmp != this; tmp = tmp.next) {
      puts("x.= "); putd(tmp.x); puts(" y.= "); putd(tmp.y); putln();
    }
  }
  ...
  private static double median(double min, double max, int n) 
  {
    ...
    double t = 0.5; //(new Random()).nextDouble();
    
    double retval;
    if (t > 0.5) {
      retval = 0.1;//log10(1.0-(2.0*(M_E12-1)*(t-0.5)/M_E12))/12.0;
    } else {
      retval = 0.1;//log10(1.0-(2.0*(M_E12-1)*t/M_E12))/12.0;
    }
  ...
  private static double uniform(double min, double max) 
  {
    ...
    double retval = 0.5; //(new Random()).nextDouble();
