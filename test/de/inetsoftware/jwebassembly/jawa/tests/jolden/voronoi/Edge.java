...
{  
  ...
  ****************************************************************/
  ...
  void outputVoronoiDiagram()
  {
    ...
    pushRing(edges, seen);
    puts("no. of edges = "); puti(edges.size()); putln();
    while (!edges.empty()) {
	...
	do {
	  ...
	  double d2 = v2.X();
	  if (d1 >= d2) {
	    puts("Dedge "); puts(v1.toString()); puts(" "); puts(v2.toString()); putln();
	    ...
	    if (v1.ccw(v2, v3) != v1.ccw(v2, v4)) {
	      ...
	      puts("Vedge "); puts(vv1.toString()); puts(" "); puts(vv2.toString()); putln();
	    }
	  }
