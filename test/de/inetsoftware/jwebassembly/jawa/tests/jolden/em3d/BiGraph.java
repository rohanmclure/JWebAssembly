...
import static de.inetsoftware.jwebassembly.jawa.util.Print.println;
...
final class BiGraph
{
  ...
  static BiGraph create(int numNodes, int numDegree, boolean verbose)
  {
    ...
    if (verbose) println("making nodes (tables in orig. version)");
    Node[] hTable = Node.fillTable(numNodes, numDegree);
    Node[] eTable = Node.fillTable(numNodes, numDegree);

    // making neighbors
    if (verbose) println("updating from and coeffs");
    ...
    if (verbose) println("filling from fields");
    for (Enumeration e = hTable[0].elements(); e.hasMoreElements(); ) {
      Node n = (Node) e.nextElement();
      n.makeFromNodes();
    }
