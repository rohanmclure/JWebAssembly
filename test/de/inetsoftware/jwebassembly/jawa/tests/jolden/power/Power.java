package de.inetsoftware.jwebassembly.jawa.tests.jolden.power;
...
public class Power 
{
  ...
  public static void main(String args[]) 
  {
    ...
    if (printMsgs) {
      puts("Power build time "); putd((end0 - start0)/1000.0); putln();
      puts("Power compute time "); putd((end1 - start1)/1000.0); putln();
      puts("Power total time "); putd((end1 - start0)/1000.0); putln();
    }
    println("Done!");
  }
  ...
  {
    println("usage: java Power [-p] [-m] [-h]");
    println("    -p (print results)");
